...
export default function PaginationHook({counterSet,pageNumber,totalPage}) {
    ...
    return (
        <div className="flex justify-center py-2 items-center gap-2 text-gray-500">
            ...
            {(pageNumber==(totalPage-2) && totalPage>2)?<div className="w-5 cursor-pointer hover:text-blue-400 transition" onClick={changeCounter}>{totalPage}</div>:(pageNumber<(totalPage-2) && totalPage>3)?<div className="w-5">...</div>:<div className="w-5"></div>}
            ...
            {(pageNumber==3 && totalPage>2)?<div className="w-5 cursor-pointer hover:text-blue-400 transition" onClick={changeCounter}>1</div>:(pageNumber>3 && totalPage>3)?<div className="w-5">...</div>:<div className="w-5"></div>}
